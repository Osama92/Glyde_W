...
export default function MapView({ onVehicleSelect }: MapView) {
  ...
  const [showPastDeliveries, setShowPastDeliveries] = useState<boolean>(false); // Toggle for past deliveries
  ...
  // Filter deliveries into active and past
  const activeDeliveries = deliveryLocations.filter((delivery) => delivery.statusId !== 4);
  const pastDeliveries = deliveryLocations.filter((delivery) => delivery.statusId === 4);
    ...
    <View style={styles.container}>
      ...
      {typeof window !== "undefined" && (
        ...
        >
          ...
          {(showPastDeliveries ? pastDeliveries : activeDeliveries).map((delivery, index) => (
            <Marker
              key={delivery.id}
          ...
          {/* Toggle for past deliveries */}
          <TouchableOpacity
            style={styles.toggleButton}
            onPress={() => setShowPastDeliveries(!showPastDeliveries)}
          >
            <Text style={styles.toggleButtonText}>
              {showPastDeliveries ? "Show Active Deliveries" : "Show Past Deliveries"}
            </Text>
          </TouchableOpacity>
        </GoogleMap>
      )}
