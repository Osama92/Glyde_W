...
//           id: doc.id,
//           vehicleNo: setShipments(doc.data().vehicleNo),
//           ...doc.data(),
//         }));
...
//               <Text style={styles.tableHeader}>VehicleNo</Text>
...
//                 <Text style={styles.tableCell}>{shipments}</Text>
...
const DashboardScreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const shipmentData = shipmentSnapshot.docs.map((doc) => ({
          id: doc.id,
          vehicleNo: doc.data().vehicleNo, // Correctly fetch vehicleNo
          ...doc.data(),
        }));
        setShipments(shipmentData); // Set the shipments state
        ...
        for (const shipment of shipmentData) {
          ...
          deliverySnapshot.forEach((doc) => {
            deliveryData.push({
              id: doc.id,
              shipmentId: shipment.id,
              vehicleNo: shipment.vehicleNo, // Pass vehicleNo to each delivery
              ...doc.data(),
            });
          });
        }
        setDeliveries(deliveryData); // Set the deliveries state
      } catch (error) {
        console.error('Error fetching data:', error);
      } finally {
        setLoading(false);
  ...
  const fetchMaterials = async (deliveryId: string, shipmentId: string) => {
    try {
      ...
      if (deliverySnapshot.exists()) {
        const deliveryData = deliverySnapshot.data();
        const materialsData = deliveryData.materials || [];
        ...
        <View style={styles.searchContainer}>
          ...
          <View style={styles.table}>
            ...
            {filteredDeliveries.map((delivery, index) => (
              ...
              >
                <Text style={styles.tableCell}>{delivery.shipmentId}</Text>
                <Text style={styles.tableCell}>{delivery.vehicleNo}</Text> {/* Use delivery.vehicleNo */}
                <Text style={styles.tableCell}>{delivery.deliveryNumber}</Text>
                <Text style={styles.tableCell}>{delivery.customer}</Text>
                <Text style={styles.tableCell}>{delivery.createdAt}</Text>
                <Text style={styles.tableCell}>{delivery.address}</Text>
