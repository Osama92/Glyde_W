...
const DashboardScreen: React.FC = () => {
    ...
    const fetchShipments = async () => {
      try {
        ...
        const shipmentData = shipmentSnapshot.docs.map(doc => ({
          ...
          createdAt: doc.data().createdAt?.toDate?.() || new Date(),
          freightCost: Number(doc.data().freightCost) || 0,
          vehicleNo: doc.data().vehicleNo || 'Unknown',
          transporter: doc.data().transporter || 'Unknown',
        }));
        setShipments(shipmentData);
      } catch (error) {
        console.error('Error fetching shipments:', error);
  ...
  // Reset vehicle selection when transporter changes
  useEffect(() => {
  ...
  const uniqueTransporters = useMemo(() => 
    [...new Set(shipments.map(s => s.transporter))].filter(Boolean).sort(),
    [shipments]
  );

  // Get available vehicles based on transporter selection
  const availableVehicles = useMemo(() => {
    if (!selectedTransporter) return [...new Set(shipments.map(s => s.vehicleNo))].filter(Boolean);
    return [...new Set(shipments
      .filter(s => s.transporter === selectedTransporter)
      .map(s => s.vehicleNo)
    )].filter(Boolean);
  }, [shipments, selectedTransporter]);

  // Filter shipments based on selections
  const filteredShipments = useMemo(() => 
    shipments.filter(s => {
      const transporterMatch = !selectedTransporter || s.transporter === selectedTransporter;
      const vehicleMatch = !selectedVehicle || s.vehicleNo === selectedVehicle;
      return transporterMatch && vehicleMatch;
    }), [shipments, selectedTransporter, selectedVehicle]);
  ...
  const trendData = useMemo(() => 
    filteredShipments.map(s => ({
      value: s.freightCost,
      label: s.createdAt.toLocaleDateString('en-US', { 
        month: 'short', 
        day: 'numeric',
        year: 'numeric'
      }),
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <LinearGradient colors={['#1e293b', '#0f172a']} style={styles.header}>
        <Text style={styles.headerTitle}>Shipment Analytics</Text>
      </LinearGradient>

      <View style={styles.filterContainer}>
        <View style={styles.filterItem}>
          <Text style={styles.filterLabel}>Select Transporter:</Text>
          <RNPickerSelect
            onValueChange={(value) => setSelectedTransporter(value === 'all' ? null : value)}
            items={[
              { label: 'All Transporters', value: 'all' },
              ...uniqueTransporters.map(t => ({ label: t, value: t }))
            ]}
            style={pickerStyles}
        ...
        <View style={styles.filterItem}>
          <Text style={styles.filterLabel}>Select Vehicle:</Text>
          <RNPickerSelect
            onValueChange={(value) => setSelectedVehicle(value === 'all' ? null : value)}
            items={[
              { label: 'All Vehicles', value: 'all' },
              ...availableVehicles.map(v => ({ label: v, value: v }))
            ]}
            style={pickerStyles}
            value={selectedVehicle || 'all'}
      ...
      <View style={styles.statsContainer}>
        <View style={styles.statCard}>
          <MaterialIcons name="local-shipping" size={24} color="#3b82f6" />
          <Text style={styles.statValue}>{totalShipments.toLocaleString()}</Text>
          <Text style={styles.statLabel}>Total Shipments</Text>
        </View>
